...
#include <stdlib.h>
/**
 * string_nconcat - it concatenates two strings up to n bytes.
 * @s1: The first string.
...
{
	char *strn;
	unsigned int countStrn1, countStrn2, bufferSize, w;
	...
	for (w = 0; w < bufferSize - 1; w++)
		w < countStrn1 ? (strn[w] = s1[w]) : (strn[w] = s2[w - countStrn1]);
